...
class PeThemeAsset {
	...
	public function registerAssets() {
		...
		// Smoothscroll
		$this->addScript("framework/js/jquery.smoothscroll.js",array(),"pe_theme_smoothscroll");

		// Waypoints
		$this->addScript("framework/js/waypoints.js",array(),"pe_theme_waypoints");

		// Parallax
		$this->addScript("framework/js/jquery.parallax-1.1.3.js",array(),"pe_theme_parallax");

		// Youtube bg player
		$this->addScript("framework/js/jquery.mb.YTPlayer.js",array(),"pe_theme_yt_bg_player");
		...
		// ajax portfolio
		$this->addStyle("framework/css/jquery.pixelentity.ajaxportfolio.css",array(),"pe_theme_ajaxportfolio");
		$this->addScript("framework/js/pe/jquery.pixelentity.scroll.js",array("jquery"),"pe_theme_scroll");
		$this->addScript("framework/js/pe/jquery.pixelentity.ajaxportfolio.js",array("pe_theme_scroll"),"pe_theme_ajaxportfolio");
		$this->addScript("framework/js/pe/jquery.pixelentity.widgets.ajaxportfolio.js",array("pe_theme_ajaxportfolio","pe_theme_widgets"),"pe_theme_widgets_ajaxportfolio");	

		// load more
		$this->addScript("framework/js/pe/jquery.pixelentity.loadmore.js",array(),"pe_theme_loadmore");
		...
		// vista slider
		$this->addScript("framework/js/pe/jquery.pixelentity.vista.js",array("pe_theme_volo_skin","pe_theme_volo_captions"),"pe_theme_vista");
		...
		// print needed theme options to header
		$defined_options = PeGlobal::$config["options"];
		$saved_options = (array) $this->master->options->all();

		$needs_print = wp_list_filter( $defined_options, array( 'jsexport' => true ) );

		if ( ! empty( $needs_print ) ) {

			$to_print = array_intersect_key( $saved_options, $needs_print );

			wp_localize_script( 'pe_theme_init', 'peThemeOptions', $to_print );
		...
		do_action("pe_theme_register_assets");
	...
	public static function addAsset($type,$res,$deps = NULL,$name = NULL,$footer = false) {
		...
		$deps = apply_filters("pe_theme_asset_{$type}_deps",$deps,$name);
		$deps = apply_filters("pe_theme_asset_{$type}_{$name}_deps",$deps);
