...
class PeThemeContent {
	...
	public function customLoopTemplate($settings,$template = "loop") {
		if (is_array($settings)) {
			$settings = (object) $settings;
		}
		$t =& $this->master;
		if ($loop = $t->data->customLoop((object) $settings)) {
			$t->template->data($settings);
			$t->get_template_part(empty($settings->template) ? $template : $settings->template);
			$this->resetLoop();
		}
	}
